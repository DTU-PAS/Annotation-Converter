...
class AnnotationConverter:
    ...
    def write_cvat(annotations, annotation_file):
        ...
        for annotation in annotations:
            ...
            for bb in bb_list:
                ...
                if label not in labels:
                    labels.append(label)
                ET.SubElement(image, "box", label=label, xtl=str(round(bb.get_x())), ytl=str(round(bb.get_y())), xbr=str(round(bb.get_x() + bb.get_width())), ybr=str(round(bb.get_y() + bb.get_height())), occluded="0", z_order="1", source="manual")
    ...
    def _cvat_to_annotation(img_xml_info):
        ...
        for bb in img_xml_info.findall("box"):
            bb_ann = BoundingBox(bb.attrib["label"], float(bb.attrib["xtl"]), float(bb.attrib["ytl"]), float(bb.attrib["xbr"]) - float(bb.attrib["xtl"]), float(bb.attrib["ybr"]) - float(bb.attrib["ytl"]))
